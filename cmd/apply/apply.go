...
func RunApply(cmd *cobra.Command, args []string) error {
	...
	if applyPlan != "" {
		...
		loaded, err := plan.LoadPlanFile(planData)
		if err != nil {
			return fmt.Errorf("failed to load plan: %w", err)
		}

		switch p := loaded.(type) {
		case *plan.MultiPlan:
			return applyMultiPlan(p, finalPassword, finalSSLMode)
		case *plan.Plan:
			return applySinglePlan(p, finalPassword, finalSSLMode)
		default:
			return fmt.Errorf("unexpected plan type loaded from file")
		}
	}
	...
	applyCfg.PlanDBHost = applyPlanDBHost
	applyCfg.PlanDBSSLMode = applyPlanDBSSLMode

	// Apply the migration
	return ApplyMigration(applyCfg, provider)
}

// applySinglePlan validates and applies a single-schema plan loaded from a JSON file.
func applySinglePlan(migrationPlan *plan.Plan, finalPassword, finalSSLMode string) error {
	...
	return ApplyMigration(applyCfg, nil)
}

// applyMultiPlan validates and applies a multi-schema plan loaded from a JSON file.
// It iterates over schemas in sorted order and applies each schema's plan individually.
func applyMultiPlan(mp *plan.MultiPlan, finalPassword, finalSSLMode string) error {
	// Validate version
	currentVersion := version.App()
	if mp.PgschemaVersion != currentVersion {
		return fmt.Errorf("plan version mismatch: plan was generated by pgschema version %s, but current version is %s. Please regenerate the plan with the current version", mp.PgschemaVersion, currentVersion)
	}
	supportedPlanVersion := version.PlanFormat()
	if mp.Version != supportedPlanVersion {
		return fmt.Errorf("unsupported plan format version: plan uses format version %s, but this pgschema version only supports format version %s. Please upgrade pgschema to apply this plan", mp.Version, supportedPlanVersion)
	}

	var hasErrors bool
	for _, schemaName := range mp.SortedSchemaNames() {
		entry := mp.Schemas[schemaName]
		...
		if err := ApplyMigration(applyCfg, nil); err != nil {
