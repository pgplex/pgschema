...
type ApplyConfig struct {
	...
	Plan            *plan.SchemaPlan // Pre-generated plan (optional, alternative to File)
	AutoApprove     bool
	NoColor         bool
...
func ApplyMigration(config *ApplyConfig, provider postgres.DesiredStateProvider) error {
	var migrationPlan *plan.SchemaPlan
	var err error
	...
	if config.Plan != nil {
		...
		migrationPlan, err = planCmd.GenerateSchemaPlan(planConfig, provider)
		if err != nil {
			return err
		}
	} else {
...
func RunApply(cmd *cobra.Command, args []string) error {
	...
	if applyPlan != "" {
		...
		loaded, err := plan.FromJSON(planData)
		if err != nil {
			return fmt.Errorf("failed to load plan: %w", err)
		}

		return applyPlanFile(loaded, finalPassword, finalSSLMode)
	}
...
// applyPlanFile validates and applies a plan loaded from a JSON file.
// It iterates over schemas in sorted order and applies each schema's plan individually.
func applyPlanFile(p *plan.Plan, finalPassword, finalSSLMode string) error {
	// Validate that the plan was generated by the same pgschema version
	currentVersion := version.App()
	if p.PgschemaVersion != currentVersion {
		return fmt.Errorf("plan version mismatch: plan was generated by pgschema version %s, but current version is %s. Please regenerate the plan with the current version", p.PgschemaVersion, currentVersion)
	}

	// Validate that the plan format version is supported (forward compatibility)
	supportedPlanVersion := version.PlanFormat()
	if p.Version != supportedPlanVersion {
		return fmt.Errorf("unsupported plan format version: plan uses format version %s, but this pgschema version only supports format version %s. Please upgrade pgschema to apply this plan", p.Version, supportedPlanVersion)
	}

	var hasErrors bool
	for _, schemaName := range p.SortedSchemaNames() {
		schemaPlan := p.Schemas[schemaName]
...
func validateSchemaFingerprint(migrationPlan *plan.SchemaPlan, host string, port int, db, user, password, sslmode, schema, applicationName string, ignoreConfig *ir.IgnoreConfig) error {
