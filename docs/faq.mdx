...
- Report bugs or request features? 👉 [GitHub Issues](https://github.com/pgplex/pgschema/issues)
...
Pre-built binaries are available on the [releases page](https://github.com/pgplex/pgschema/releases).
...
### Can pgschema manage table data?

Only for reference tables. List the table under `[data]` in [`pgschema.toml`](/cli/config), run `dump` to export its rows to a CSV file loaded by a [`\copy`](/syntax/copy) directive, and from then on `plan` generates the `INSERT`, `UPDATE`, and `DELETE` statements needed to match the file. Tables that are not listed are never touched, and `INSERT` statements for them are discarded while building the desired state, as before. See [Reference Data](/workflow/reference-data). For backfills and other one-off changes to user data, see [Data Migration](/workflow/data-migration).
...
## Why is pgschema free?

We have no current plans to charge for pgschema.
