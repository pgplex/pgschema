...
- A change to a primary key value is an `INSERT` of the new row and a `DELETE` of the old one, with the delete ordered after any updates that re-point child rows
