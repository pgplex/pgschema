...
- A change to the primary key columns themselves clears the table with a single `DELETE FROM` and inserts every row again, since the old key may no longer exist once the DDL has run
