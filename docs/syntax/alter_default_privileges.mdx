...
<Warning>
`IN SCHEMA` is required. `ALTER DEFAULT PRIVILEGES` without `IN SCHEMA` sets a database-wide default for the role, which is outside the schema `pgschema` manages. `plan` prints a warning for such a statement and skips it, so apply it outside of `pgschema`. See [Unsupported](/syntax/unsupported#global-default-privileges).
</Warning>
