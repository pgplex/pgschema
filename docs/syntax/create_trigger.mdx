...
- **Enabled/disabled state**: A non-default trigger state (`ALTER TABLE ... DISABLE TRIGGER`, `ENABLE REPLICA TRIGGER`, `ENABLE ALWAYS TRIGGER`) is tracked and re-applied; returning to the default emits `ALTER TABLE ... ENABLE TRIGGER`. PostgreSQL does not allow disabling triggers on views.
...
- For enabled/disabled state changes: `ALTER TABLE table_name { ENABLE | ENABLE REPLICA | ENABLE ALWAYS | DISABLE } TRIGGER trigger_name;`
