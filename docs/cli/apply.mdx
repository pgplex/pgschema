...
1. Compare it with the current database state of the target schema, including the rows of reference tables declared with [`\copy`](/syntax/copy) directives
...
- **Transactional mode** (default): Changes run in transaction groups with automatic rollback of the failing group
- **Non-transactional mode**: Some operations (like `CREATE INDEX CONCURRENTLY`) run outside transactions
- **Isolated steps**: Online rewrites split certain steps into their own transactions - for example, `VALIDATE CONSTRAINT` always commits separately from the preceding `ADD ... NOT VALID` so the validation scan doesn't run under the lock taken by `ADD`
