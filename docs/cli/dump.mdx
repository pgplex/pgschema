...
<ParamField path="--multi-file" type="boolean" default="false">
  ...
  - One `data/<table>.csv` file per table listed in [`pgschema.toml`](/cli/config), declared last
...
<ParamField path="--file" type="string">
  ...
  Required when [`pgschema.toml`](/cli/config) lists reference tables, since their CSV files are written to `data/` next to the output file.
</ParamField>
...
Tables listed under `[data]` in [`pgschema.toml`](/cli/config) have their rows dumped to CSV files alongside the schema. See [Reference Data](/workflow/reference-data) for the workflow and the [example](#reference-data) below.
...
With a `pgschema.toml` in the current directory:
...
Listed tables get a `\copy` directive after all other objects in `schema.sql`:
...
and their rows go to `data/country.csv` beside it:
...
With `--multi-file`, the directive is appended to the main file after every `\i` include. Without a `pgschema.toml`, or with no matching tables, dump output is unchanged and contains no rows. See [\copy](/syntax/copy) for the format and [Reference Data](/workflow/reference-data) for the workflow.
