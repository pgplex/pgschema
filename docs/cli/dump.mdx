...
<ParamField path="--file" type="string">
  ...
  Required when [`pgschema.toml`](/cli/config) lists config tables, since their CSV files are written to `data/` next to the output file.
</ParamField>
...
Tables listed under `[data]` in [`pgschema.toml`](/cli/config) have their rows dumped to CSV files alongside the schema. See [Config Data](/workflow/config-data) for the workflow and the [example](#config-data) below.
...
With `--multi-file`, the directive is appended to the main file after every `\i` include. Without a `pgschema.toml`, or with no matching tables, dump output is unchanged and contains no rows. See [\copy](/syntax/copy) for the format and [Config Data](/workflow/config-data) for the workflow.
