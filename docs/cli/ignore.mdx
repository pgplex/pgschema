...
<Note>
The `.pgschemaignore` file is automatically loaded when present in the current directory. Project configuration that is not about exclusion, such as the list of [config tables](/workflow/config-data), lives in [`pgschema.toml`](/cli/config). A table cannot be both ignored here and listed there.
</Note>
