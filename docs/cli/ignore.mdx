...
<Note>
Unknown sections or keys (for example a misspelled `patterhs`) are rejected with an error rather than silently ignored, so a typo can't leave a section without effect.
</Note>
