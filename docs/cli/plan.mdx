...
1. Compare the two states, including the rows of reference tables declared with [`\copy`](/syntax/copy) directives
...
When the schema declares rows with [`\copy`](/syntax/copy) directives, row changes are listed under their table and emitted as `INSERT`, `UPDATE`, and `DELETE` steps of type `table.data`:
...
See [Reference Data](/workflow/reference-data) for details.
...
[`\copy`](/syntax/copy) directives are resolved the same way and load reference rows from CSV files:
