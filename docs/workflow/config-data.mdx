...
1. `DELETE` statements for rows whose secondary unique value (a `UNIQUE` constraint or unique index other than the primary key) is reused by an inserted row run first, child tables before parent tables, so the insert does not collide
...
Changing a primary key value is a `DELETE` plus an `INSERT`. A referenced row can be re-keyed in one plan as long as the managed rows that reference it are updated in the same plan, and a row can be re-keyed while keeping its unique values. A row that is both referenced by child rows and keeps a unique value cannot be re-keyed in one plan; edit the plan by hand for that case.
