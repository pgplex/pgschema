...
1. `INSERT` statements run first, parent tables before child tables
2. `UPDATE` statements run next, parent tables before child tables, so a child re-pointed at a new parent row finds it
3. `DELETE` statements run last, child tables before parent tables, after the updates that moved any remaining references away

Changing a primary key value is a `DELETE` plus an `INSERT`, so a referenced row can be re-keyed in one plan as long as the managed rows that reference it are updated in the same plan.
