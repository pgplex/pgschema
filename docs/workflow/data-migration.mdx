...
<Note>
Config tables (lookups, settings, feature flags) are the exception: list them in [`pgschema.toml`](/cli/config), keep their rows in CSV files loaded by [`\copy`](/syntax/copy), and pgschema plans the row changes for you. See [Config Data](/workflow/config-data). This page covers one-off changes to data that pgschema does not own.
</Note>
