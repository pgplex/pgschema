...
```sql
CREATE FUNCTION accept_extracted_recipes(recipe_ids uuid[])
RETURNS SETOF recipe_user_save   -- depends on the recipe_user_save table existing
LANGUAGE plpgsql AS $$ ... $$;
```
...
The `main.sql` generated by `dump --multi-file` orders its `\i` directives by dependency rather than by directory, so a function that uses a table's or view's row type is included after that relation even though it lives under `functions/`. You need to manage ordering yourself when you write or edit `main.sql` by hand, for example with [Custom Organization](#custom-organization). In that case, make sure tables (and any other referenced objects) are included before the functions, views, and procedures that depend on them:

```sql
-- main.sql — referenced objects first
\i tables/recipe_user_save.sql
-- ... other tables ...
\i functions/accept_extracted_recipes.sql
```
...
├── triggers/
├── types/
└── data/                  # CSV files for tables listed in pgschema.toml
```

Rows of [reference tables](/workflow/reference-data) live as CSV files under `data/`, loaded by `\copy` directives at the end of `main.sql`.
