...
The default `dump --multi-file` output orders its `\i` directives by dependency rather than by directory, so a function that uses a table's or view's row type is included after that relation even though it lives under `functions/`. This only comes up when you reorganize files yourself. If you use [Custom Organization](#custom-organization), make sure tables (and any other referenced objects) are included before the functions, views, and procedures that depend on them:
