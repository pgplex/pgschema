...
The default `dump --multi-file` output already orders directives correctly (types → tables → views → functions → indexes), so this only comes up when you reorganize files yourself. If you use [Custom Organization](#custom-organization), make sure tables (and any other referenced objects) are included before the functions, views, and procedures that depend on them:
