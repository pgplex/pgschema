...
The tool supports PostgreSQL versions 14-18 and handles most common schema objects (tables, indexes, functions, views, constraints, etc.), plus the rows of reference tables (lookup and configuration data) listed in `pgschema.toml`.
...
- **Reference data**: Rows of tables listed under `[data]` in `pgschema.toml`, kept in CSV files loaded by `\copy` directives and diffed by primary key
