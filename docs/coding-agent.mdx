...
The tool supports PostgreSQL versions 14-18 and handles most common schema objects (tables, indexes, functions, views, constraints, etc.), plus the rows of config tables (lookups, settings, feature flags) listed in `pgschema.toml`.
...
- **Config data**: Rows of tables listed under `[data]` in `pgschema.toml`, kept in CSV files loaded by `\copy` directives and diffed by primary key
