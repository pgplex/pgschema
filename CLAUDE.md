...
- **Reference Data**: Rows of tables listed under `[data]` in `pgschema.toml`, kept in CSV files loaded by `\copy` directives, diffed by primary key into INSERT/UPDATE/DELETE (`internal/diff/data.go`, `internal/include/copy.go`, `internal/postgres/copy.go`, `internal/dump/data.go`)
...
- `ir/data.go` - DataConfig (`[data]` section of `pgschema.toml`), row loading for reference tables
...
- `data/` (7) - reference data; each case also has `pgschema.toml` and a `data/` directory of CSV files loaded by `\copy` in `new.sql`
